	...
	<div class="content">
		<div class="checktab">
			<?php
			                ...
			                include ("methods.php");
			                $cart2 = new Cart;?>
			<h1>確認訂單</h1><?php echo '餐點總數：'.$cart2->total_items2() ."<br/>";
			                if($cart2->total_items2() > 0){ $cartItems = $cart2->contents2();
			                    echo "訂單內容：" ."<br>";
			                foreach($cartItems as $item){echo $item["meal"];?><?php echo "&nbsp".'$'.$item["price"];?>(<?php echo $item["qty"]; ?>) <?php echo '$'.$item["subtotal"] ."<br/>";}
			                }?>
			<div class="total">
				<?php echo "<br>" .'總金額：'.'$'.$cart2->total2() ."</br/>";?>
			</div>
		</div>
		<div class="keep">
			<a class="btn btn-block btn-info" href="../城中店家頁面/index.php"><input name="btn" type="button" value="繼續購物"></a>
		</div>
		...
		<div class="id">
			...
			<div class="place">領餐地點 : <input checked id="place" name="place" required="" type="radio" value="711前">711前<input id="place" name="place" type="radio" value="E棟1樓">E棟1樓 <input id="place" name="place" type="radio" value="D棟1樓">D棟1樓 <br>
			</div>
				備註 :&nbsp&nbsp
